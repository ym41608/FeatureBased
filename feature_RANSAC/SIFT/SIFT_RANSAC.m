% SIFT matching and RANSAC
%
% Usage:
%   [P Q fail] = SIFT_RANSAC(Marker, Img, in_mat)
%
% Inputs:
...
% Dec, 2015
function [P, Q, fail] = SIFT_RANSAC(Marker, Img, in_mat, nor_mat)
  P = []; Q = []; fail = false;
  % add path
  addpath(genpath('./SIFT'));
  
  % SIFT
  [frames1,descr1,gss1,dogss1] = sift( Marker, 'Verbosity', 0 );
  descr1=uint8(512*descr1);
  [frames2,descr2,gss2,dogss2] = sift( Img, 'Verbosity', 0 );
  descr2=uint8(512*descr2);
  matches=siftmatch( descr1, descr2 );

  % Normalize pixel coordinates and change direction of y axis.
  frames1 = nor_mat * [frames1(1:2, :); ones(1, size(frames1,2))];%[((frames1(1,:)-w1/2)./(w1/2))*marker_w; -((frames1(2,:)-h1/2)/(h1/2)*marker_h)];
  
  if (size(frames1, 2) == 0 || size(frames2, 2) == 0 | size(matches, 2) == 0)
    fail = true;
  else
    % RANSAC
    [P, Q, ~] = GetInlier(frames1(1:2, :), frames2(1:2, :), matches, 10, 1000, in_mat);
    fail = false;
  end
end
