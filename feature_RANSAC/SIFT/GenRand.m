function T = GenRand( tarframes, scrframes, matches, corres)
% Get random samples and calculate the perspective transformation matrix
%
% Usage:
%   T = GenRand( tarframes, scrframes, matches, corres)
%
% Inputs:
...
% --- Construct A & B
bi = 1; ei = corres;
A(bi:ei,1:2) = tarframes(:,matches(1, C(:)))';
A(bi:ei,3)   = ones(corres, 1);
A(bi:ei,7)   = -1*tarframes(1, matches(1, C(:))).*scrframes(1, matches(2, C(:)));
A(bi:ei,8)   = -1*tarframes(2, matches(1, C(:))).*scrframes(1, matches(2, C(:)));
bi = corres+1; ei = corres+corres;
A(bi:ei,4:5) = tarframes(:, matches(1, C(:)))';
A(bi:ei,6)   = ones(corres, 1);
A(bi:ei,7)   = -1*tarframes(1, matches(1, C(:))).*scrframes(2, matches(2, C(:)));
A(bi:ei,8)   = -1*tarframes(2, matches(1, C(:))).*scrframes(2, matches(2, C(:)));
B(:) = [scrframes(1, matches(2, C(:))), scrframes(2, matches(2, C(:)))];
